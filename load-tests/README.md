...
extension's Protobuf + Schema Registry support has been fully implemented since v2.1.0.
...
- Sanity check: Visa's publicly-cited global peak capacity is ~65,000 TPS across *all* issuers
  combined — a single 24M-customer issuer at ~1,000–2,500 TPS peak is a proportionate slice of that,
  not an inflated number.

These numbers size each scenario below; they are **not** a claim about what this deployment's current
Kafka topology (6 partitions on `transactions.raw`, 6 consumer threads — `KafkaConfig.java`) can
...
**03-spike** — the hardest test for Kafka absorption. A sudden burst to the estimated
...
**04-fraud-rules** — the most realistic simulation, at the estimated peak-hour rate: 60% clean
traffic, 20% high-value (triggers `AMOUNT_THRESHOLD`), 20% high-risk merchant category (triggers
`HIGH_RISK_MERCHANT_CATEGORY` — replaces an earlier "unrecognised device" bucket, since
