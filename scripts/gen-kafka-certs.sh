...
#
# In production: replace these self-signed certs with ones issued by the Capitec
...
openssl req -new -x509 \
  ...
  -days "$VALIDITY_DAYS" \
  -subj "/C=ZA/ST=Western Cape/L=Cape Town/O=Capitec Bank/OU=Fraud Engine/CN=FraudEngine-KafkaCA"
...
for broker in "${BROKERS[@]}"; do
  ...
  openssl req -new \
    ...
    -subj "/C=ZA/O=Capitec Bank/OU=Fraud Engine/CN=$broker"
...
echo "  For production, obtain certs from the Capitec internal CA and"
