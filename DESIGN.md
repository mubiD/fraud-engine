...
Priority 4 (`BLACKLISTED_MERCHANT`) is deliberately absent — the rule was removed. Rationale: this is a strictly post-authorisation system (§3) — the transaction has already gone through by the time any rule evaluates it, so the rule could never block the fraud it detects, only report on it after the fact. Its `CRITICAL` severity and near-standalone likelihood ratio (see §5's Risk scoring) correctly captured "how certain is this evidence," but implicitly oversold "how much can this system do about it" — the honest answer being: improve a compliance report and prioritise an analyst's review queue, not prevent loss, unless some system outside this one consumes `transactions.flagged` and acts on it fast enough to matter, which nothing here demonstrates. The underlying data path — `blacklisted_merchants` (§6), `BlacklistedMerchantRepository`, and `ReferenceDataCache.getBlacklistedMerchantIds()` (Caffeine, 5-min TTL) — was deliberately left in place rather than deleted: it still populates `EvaluationContext.blacklistedMerchantIds` on every evaluation, just unread by any rule now, specifically so the cache-staleness and multi-instance-invalidation questions it raises (a 5-min TTL is a materially different problem for a single-instance app than for `helm/values-prod.yaml`'s `replicaCount: 2`, where local-cache eviction on write only fixes the pod that received the write) remain inspectable as a live, running example rather than an archived one.
...
- All 13 rules run synchronously on the Kafka consumer thread. Most of `EvaluationContextBuilder`'s data is now an in-memory Interactive Query against the Kafka Streams state store (see above) rather than a live DB round-trip, but `blacklistedMerchantIds`/merchant location (Caffeine-cached, not DB-per-call) and `customerBaselineTransactions` (`CustomerAmountAnomalyRule`'s 90-day baseline, still a live Postgres query, gated behind its own `enabled` flag so it doesn't cost anything when unused) remain outside the streaming path. Closing that last query is the remaining piece of what would need to change (not the rules themselves, which are pure in-memory logic) to support a real-time, pre-authorisation decision path. See [§11](#11-known-drawbacks--production-considerations).
...
|---|---|
| **Post-authorisation only** — no synchronous, blocking decision path exists before a transaction is authorised. The rules themselves are pure in-memory logic and could run in a sub-100ms budget. | Partially addressed: `recentCustomerTransactions`/`dailySpendTotal` are now served by a Kafka Streams state store (`com.fraudengine.streams`, §5) instead of live Postgres queries, covering `VelocityRule`, `CrossMerchantVelocityRule`, `DuplicateTransactionRule`, `CardCloningRule`, `MultiChannelAnomalyRule`, `GeographicAnomalyRule`, and `CumulativeSpendingRule`. Still open: `CustomerAmountAnomalyRule`'s 90-day baseline and the blacklist/merchant-location cache-miss path remain live Postgres queries, and the largest architectural gap by design scope — nothing here changes the overall async, post-authorisation ingestion model itself (§3) — remains. New, narrower gap introduced by this change: the Streams app is an independent consumer group from `TransactionConsumer` on the same topic, so it can lag behind it under restart/load; a read in that window under-counts rather than fails, which is a false-negative risk this doesn't close (see §5's Known Limitation), only makes observable via a lag metric. |
