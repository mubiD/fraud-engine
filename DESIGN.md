...
`RuleEngine` holds every Spring-registered `FraudRule` bean, filters to enabled ones, sorts by priority, evaluates each against a shared `EvaluationContext`, and aggregates the violations into a `FraudAssessment`. Adding a rule is: implement `FraudRule`, annotate `@Component` — zero changes to `RuleEngine` or any existing rule (Open/Closed). Each rule's `evaluate()` call is individually fault-isolated: a rule that throws an unanticipated `RuntimeException` (found live 2026-09-16 — until then, one rule's bug aborted evaluation for all 12) is logged, counted (`fraud.rule.error.total`, tagged by rule name), and treated as a pass rather than propagating — the same "one bad input shouldn't crash everything else" posture `CustomerActivityProcessor` already applies to a malformed Kafka record, applied here so one rule's bug degrades an assessment to "11 rules ran" instead of failing it outright and routing the whole transaction through Kafka's retry/DLT path.
...
`RuleProperties.validate()` (a `@PostConstruct` check) asserts that every rule's configured window is within `context-lookback-minutes` — a rule whose window is wider than the lookback would silently under-count, since `recentCustomerTransactions` simply wouldn't contain the data it needs. This check is expressed as two exhaustive maps of rule name → window — one in minutes for every rule but `DuplicateTransactionRule`, one in seconds for that rule's two windows specifically (found live 2026-09-16: its seconds-based windows were missing from startup validation entirely, silently exempt from the same guarantee every other rule got) — specifically so adding a new window-based rule later means adding one line to the appropriate map, not writing a new one-off `if`.
...
**Not in scope:** `DeviceFingerprintRule` needs no dedicated state in this topology — its own recent-fingerprint lookup already rides along on `recentCustomerTransactions` above, the same list every other rule shares. What still limits it in practice isn't this topology; see §11's `DeviceFingerprintRule` row: `device_fingerprint` is now a real field (added as field 12) wired end-to-end through this exact streaming path, but the rule stays practically inert until the external upstream producer this repo doesn't own actually starts populating it. Merchant location (`ReferenceDataCache`) also stays outside this topology — it's Caffeine-cached, eagerly warmed at startup, not per-transaction state — see the sub-100ms row in §11.
...
| `RuleProperties.validate()` as two exhaustive maps (minutes, seconds) | A missed window-vs-lookback check is one omitted map entry, not an easy-to-miss `if` |
...
| **`DeviceFingerprintRule` cannot fire via the Kafka ingestion path.** Found live 2026-09-10: `transaction_event.proto` (the real Kafka wire format) carried no `device_fingerprint` field, so every transaction ingested via `TransactionConsumer` always had `deviceFingerprint = null` — the rule's `knownFingerprints` set could never be non-empty, so it could never violate. HIGH severity, likelihood ratio 160.0 (one of the highest-calibrated signals in `ScoringProperties`), fully implemented and tested, silently inert on the Kafka path. Undocumented until now. | Partially addressed 2026-09-10: added `device_fingerprint` as field 12 (additive, Schema-Registry-compatible) and wired it through `ProtoMapper`, `RecentTransactionRecord`, `CustomerActivityProcessor`, and `EvaluationContextBuilder.toTransaction` — the pipe is ready end-to-end, streaming path included. Still open: `transactions.raw` is published by an external upstream system this repo doesn't own (the authorisation switch/core banking platform, per §3); the rule stays inert in practice until that producer actually starts populating the field. Not something this codebase can finish alone. |
